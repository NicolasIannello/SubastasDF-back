...
const getAdjudicados= async(req,res=response)=>{
    const userDB = await Usuario.findById(req.uid);
    const adjudicadosDB = await Lote.aggregate([
        { "$match": { ganador:userDB.mail } },
        { $lookup: {
            from: "eventolotes",
            localField: "uuid",
            ...
            "pipeline": [ 
                ...
                { $project: {
                    __v: 0,"evento.__v": 0,"evento._id": 0,"evento.categoria": 0,"evento.estado": 0,"evento.eventos": 0,"evento.fecha_cierre": 0,"evento.fecha_inicio": 0,
                    "evento.grupo": 0,"evento.home": 0,"evento.hora_cierre": 0,"evento.hora_inicio": 0,"evento.inicio_automatico": 0,"evento.modalidad": 0,
                    "evento.mostrar_ganadores": 0,"evento.mostrar_ofertas": 0,"evento.mostrar_precio": 0,"evento.publicar_cierre": 0,"evento.segundos_cierre": 0,"evento.uuid": 0,
                    "evento.visitas": 0,
                } },        
            ],
        }},
        { $project: {
            __v: 0,"evlot.__v": 0,"evlot._id": 0,"evlot.uuid_lote": 0,
        } },
    ]).collation({locale: 'en'});
